...
#bs.proof([
    "$arrow.r$": Suppose $rho = sum_i p_i sn.ket(psi_i) sn.tek(psi_i)$, then:
    $
    ...
    $
    #sn.hline()
    "$arrow.l$": $rho$ is an operator (i.e. a hermitian matrix) $arrow.r.squiggly$ by spectral theorem there exist eigenvalues $lambda_j$ and corresponding orthonormal eigenvecetors $sn.ket(phi_j)$ such that: 
     $ 
...
#text(blue)[
    ...
    The sets ${sn.ket(accent(phi_i, tilde))}$ and ${sn.ket(accent(phi_j, tilde))}$ generate the same density matrix if and only if  $ sn.ket(accent(phi_i, tilde)) = sum_j u_(i j) sn.ket(accent(phi_j, tilde)) $ for some unitary matrix ($u_(i j)$),
]\
Sketch of proof: \
$"arrow.l"$: Insert definitions.\
$"arrow.r"$: Use the spectral decomposition of the density matrix:  $ rho = sum_k lambda_k sn.ket(chi_k) sn.tek(chi_k) "with" sn.bra(chi_k|chi_k) = delta_(k l), $ 
set $sn.ket(accent(chi_k, tilde)) = sqrt(lambda_k) sn.ket(chi_k)$, express $sn.ket(accent(phi_i, tilde)) = sum_k v_(i k) sn.ket(accent(chi_k, tilde))$ for some complex coefficients $v_(i k)$.\
Then  $ underbrace(sum_k sn.ket(accent(chi_k, tilde)) sn.tek(accent(chi_k, tilde)))_(#text(gray)[$sum_(k,l) #text(green)[$delta_(k,l)$] sn.ket(accent(chi_k, tilde)) sn.tek(accent(chi_l, tilde))$]) = rho = sum_i sn.ket(accent(phi_i, tilde)) sn.tek(accent(phi_i, tilde)) = sum_(k,l)(#text(green)[$sum_i v_(i k)v_(i l)^*$]) sn.ket(accent(chi_k, tilde)) sn.tek(accent(chi_l, tilde)) $ 
This equality can only be satisfied (since the $sn.ket(accent(chi_k, tilde))$ are orthogonal and thus $sn.ket(accent(chi_k, tilde))sn.tek(accent(chi_l, tilde))$ linearly independent) if  $ #text(green)[$delta_(k,l) = sum_i v_(i k)v_(i l)^*$] #text(gray)[$= (V^*V^T)_(l k) = (V*V^dagger)^*_(l k)$] $  in other words, if ($v_(i k)$) is a unitary matrix.\
By the same arguments, $sn.ket(phi_j) = sum_k w_(j k)sn.ket(accent(chi_k, tilde))$ for a unitary matrix ($w_(j k)$). Thus 
$ sn.ket(accent(phi_i, tilde)) = sum_k v_(i k)sn.ket(accent(chi_k, tilde)) = sum_(k,j) v_(i k) w_(j k)^* sn.ket(accent(phi_j, tilde)) = sum_j (v w^dagger)_(i j) sn.ket(accent(phi_j, tilde)), $  and $v*w^dagger$ is (as product of unitary matrices) again unitary.\

The Bloch sphere picture for qubits can be generalized to mixed states by the following representation  $ rho = (I + arrow(r) * arrow(sigma))/2 $  with $arrow(r) in bb(R), ||arrow(r)|| <= 1,$ the Bloch vector of $rho$ (Coincides with hitherto definition in case $rho = sn.ket(psi)sn.tek(psi)$) 
\subsection(The reduced density operator)
#text(blue)[Definition (partial trace): Let $n_1, n_2 in bb(N).$\
...
#text(gray)[$ tr_1[alpha M_1 times.circle M_2 + beta N_1 times.circle N_2] = alpha tr_1 [M_1 times.circle M_2] + beta tr_2[N_1 times.circle N_2] $]
...
Define the #underline("reduced density operator") for a System A by  $ rho^A = tr_B[rho^(A B)] $  and analogously  $ rho^B = tr_A[rho^(A B)] $ .
Examples:

    - For any quantum states $sn.ket(a_1), sn.ket(a_2) in bb(A)$ and $sn.ket(b_1), sn.ket(b_2) in bb(B)$:
     $ tr_B[sn.ket(a_1)sn.tek(a_2) times.circle sn.ket(b_1)sn.tek(b_2)] = sn.ket(a_1)sn.tek(a_2) dot underbrace(tr[sn.ket(b_1)sn.tek(b_2)])_(sn.bra(b_2|b_1))= sn.ket(a_1)sn.tek(a_2) dot sn.bra(b_2|b_1) $ 
    ...
    Then  $ tr_B[rho times.circle sigma] = rho dot underbrace(tr[sigma])_(1) = rho $ 
     $ tr_A[rho times.circle sigma] = sigma dot underbrace(tr[rho])_(1) = sigma$ 
    - $rho^(A B) ? sn.ket(psi)sn.tek(psi)$ with $sn.ket(psi) = 1/sqrt(2) (sn.ket(00) + sn.ket(11))$ (Bell state)\
    Expanding $rho^(A B)$ leads to
     $ rho^(A B) = 1/2 (sn.ket(00) + sn.ket(11))(sn.tek(00) + sn.tek(11)) = 1/2 (sn.ket(00)sn.tek(00) + sn.ket(00)sn.tek(11) + sn.ket(11)sn.tek(00) + sn.ket(11)sn.tek(11)) $ 
    Then 
     $ rho^A = tr_B[rho^(A B)] = 1/2 (sn.ket(0)sn.tek(0) dot underbrace(sn.bra(0|0))_(1) + sn.ket(0)sn.tek(1) dot underbrace(sn.bra(0|1))_(0) + sn.ket(1)sn.tek(0) dot sn.bra(1|0) + sn.ket(1)sn.tek(1) dot sn.bra(1|1)) = 1/2 ((sn.ket(0)sn.tek(0) + sn.ket(1)sn.tek(1)) = I/2 $ 
    \textit(Note: composite system is in the "pure state" $sn.ket(psi)$, whereas the subsystem is described by the "mixed state" $I/2$ (Indeed a mixed state: $tr[(I/2)^2] =  1/4 tr [I] = 1/2 < 1$))

    Motivation/justification for the partial trace:\
    Let M be any observable on subsystem A, then we want that $rho^A$ yields the same statistics for measuring M as $rho^(A B)$ for measuring $M times.circle I$.\
    In particular  $ sn.bra(M) = tr[M dot rho^A] =^! tr[(M times.circle I) rho^(A B)] = sn.bra(M times.circle I) $ 
    ...
    Application to quantum teleportation: Why does quantum teleportation not allow for fester than light communication via the instantaneous wavefunction collapse?\
    Recall the corresponding quantum circuit:

    (First two qubits has _Alice_ and the last qubit has _Bob_)\
At $sn.ket(psi_3)$, _Alice_ has completed her measurements (her qubits have "collapsed"), but _Bob_ does not know her measurement results yet. \

Intermediate state $sn.ket(psi_2)$: (see above)
 $ sn.ket(psi_2) ? 1/2 (sn.ket(00) (alpha sn.ket(0) + beta sn.ket(1)) + sn.ket(01) (alpha sn.ket(1) + beta sn.ket(0) + sn.ket(10) (alpha sn.ket(0) - beta sn.ket(1)) + sn.ket(11) (alpha sn.ket(1) - beta sn.ket(0))) $ 
...
Corresponding density matrix of ensemble ${1/4, sn.ket(phi_j)}_(j=1...4)$;
 $ rho^(A B) = 1/4 sum^4_(j=1) sn.ket(phi_j)sn.tek(phi_j) =  $  $ 1/4 (sn.ket(00) sn.tek(00) times.circle (alpha sn.ket(0) + beta sn.ket(1))(alpha^* sn.tek(0) + beta^*sn.tek(1))  $  $ +
sn.ket(01) sn.tek(01) times.circle (alpha sn.ket(1) + beta sn.ket(0))(alpha^* sn.tek(1) + beta^*sn.tek(0))  $  $ + 
sn.ket(10) sn.tek(10) times.circle (alpha sn.ket(0) - beta sn.ket(1))(alpha^* sn.tek(0) - beta^*sn.tek(1))  $  $ + 
sn.ket(11) sn.tek(11) times.circle (alpha sn.ket(1) - beta sn.ket(0))(alpha^* sn.tek(1) - beta^*sn.tek(0))) $ 
Reduced density operator describing _Bob_'s qubit:
 $ rho^A = tr[rho^(A B)] = 1/4((alpha sn.ket(0) + beta sn.ket(1))(alpha^* sn.tek(0) + beta^*sn.tek(1)) +(alpha sn.ket(1) + beta sn.ket(0))(alpha^* sn.tek(1) + beta^*sn.tek(0)) + $  $  (alpha sn.ket(0) - beta sn.ket(1))(alpha^* sn.tek(0) - beta^*sn.tek(1)) + (alpha sn.ket(1) - beta sn.ket(0))(alpha^* sn.tek(1) - beta^*sn.tek(0))) =  $ 
 $ = 1/4 ( 2 ( |alpha|^2 + |beta|^2) sn.ket(0)sn.tek(0) + 2 (underbrace(|alpha|^2 + |beta|^2)_1 sn.ket(1)sn.tek(1) = 1/2(sn.ket(0)sn.tek(0) + sn.ket(1)sn.tek(1)) = I/2 $ 
\begin(center)
